...
The engine is Tattle's four-step loop: sample, annotate, keyword-expand, analyze. The most deployment-tested method in the corpus, proven in three NGO pilots [Manual Evaluations of AI in the Social Sector](https://tattle.co.in/blog/2026-05-28-how-to-do-manual-evaluations/). Four constraints sit around it, each doing one job:
...
- Language robustness: every automated evaluation is calibrated against a per-language human gold set (a small human-labeled reference dataset), because judge calibration in English doesn't transfer — pairwise judges prefer English answers regardless of correctness and are worst on culturally grounded subjects [Fairness or Fluency?](https://arxiv.org/abs/2601.13649); [Pradhan et al., 2025](https://arxiv.org/abs/2509.12382).
...
- Codification: every round ends in validators plus a Social-Sector [RiskCard](https://arxiv.org/abs/2303.18190); re-evaluation measures whether the guardrails worked.

Two builds show the tier split is buildable. [Kaapi Guardrails](https://projecttech4dev.org/kaapi-guardrails-tattle-tech4dev/) turned evaluation findings into four running validators on the Guardrails AI framework (a slur check, a personal-information detector, a gender-assumption check, a ban list). [Guardrails×MLflow](https://guardrailsai.com/blog/guardrails-mlflow) runs the same Hub validators as evaluation scorers, turning safety regressions into continuous-integration tests.
