...
[PLACEHOLDER (screenshot): a real conference-notes Telegram chat. The raw material a live bloq publishes.]
...
[PLACEHOLDER (screenshot): the /livesession start bot reply, confirmation plus the live page URL]
...
[PLACEHOLDER (screenshot): the /livesession usage refresher reply the bot sends after a bare or misspoken command]
...
[PLACEHOLDER (screenshot): /livesession status output showing entry count, runtime, and the 8-hour warning if one of the piloted sessions ever earned it]
...
[PLACEHOLDER (screenshot): the "Entry #N added." bot reply, the mode confession]
...
[PLACEHOLDER (screenshot): the /bloq feed with a live session sitting between regular bloqs, indistinguishable until you read the card]
