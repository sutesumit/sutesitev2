  ...
  return (
    <footer className="footer fixed w-full bottom-0 z-10">
      <div className='container flex justify-between p-2 border-t-2 border-slate-300 dark:border-slate-700 backdrop-blur-3xl lowercase'>
        <div className="current-time flex flex-wrap items-center gap-3">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="last-visit group flex items-center gap-1 text-xs opacity-50 cursor-pointer hover:opacity-100 transition-all duration-300">
                {lastVisitorLocation && <CheckCheck className='h-4 w-4 group-hover:text-blue-500 dark:group-hover:text-blue-400' />} <ScrambleText text={displayText} />
              </div>
            </TooltipTrigger>
            {visitorCount && (
              <TooltipContent>
                <p>last visitor/<ScrambleText text={visitorCount.toString()} /></p>
              </TooltipContent>
            )}
          </Tooltip>
        </TooltipProvider>
