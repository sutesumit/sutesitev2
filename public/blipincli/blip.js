...
|.{1,${columnWidth - 1}}([${breaks}]|$)|[^${breaks}]+?([${breaks}]|$)`,
        ...
        const lines = columnText.match(regex4) || [];
...
var require_command = __commonJS({
  "node_modules/commander/lib/command.js"(exports) {
    ...
    var process17 = __require("node:process");
...
-  already used by option '${matchingOption.flags}'`);
      ...
      _prepareUserArgs(argv, parseOptions) {
        ...
        if (argv === void 0 && parseOptions.from === void 0) {
          ...
          const execArgv = process17.execArgv ?? [];
...
var require_emoji_regex = __commonJS({
  "node_modules/ora/node_modules/emoji-regex/index.js"(exports, module) {
    module.exports = () => {
...
var require_string_width = __commonJS({
  "node_modules/string-width/index.js"(exports, module) {
    ...
    var emojiRegex4 = require_emoji_regex4();
    ...
    module.exports = stringWidth4;
    module.exports.default = stringWidth4;
  }
});
  ...
  "node_modules/cli-table3/src/utils.js"(exports, module) {
    ...
    function strlen(str) {
      ...
      return split.reduce(function(memo, s) {
        return stringWidth4(s) > memo ? stringWidth4(s) : memo;
      }, 0);
    }
...
var IS_USER_ROOT = process11.getuid ? !process11.getuid() : false;
...
var DEFAULT_USER_UID = process12.geteuid ? process12.geteuid() : -1;
var DEFAULT_USER_GID = process12.getegid ? process12.getegid() : -1;
var DEFAULT_TIMEOUT_SYNC = 1e3;
var IS_POSIX = !!process12.getuid;
var IS_USER_ROOT2 = process12.getuid ? !process12.getuid() : false;
...
var IS_LINUX = process13.platform === "linux";
var IS_WINDOWS = process13.platform === "win32";
    ...
    this.exit = (signal) => {
      ...
      if (signal) {
        if (IS_WINDOWS && (signal !== "SIGINT" && signal !== "SIGTERM" && signal !== "SIGKILL")) {
          process14.kill(process14.pid, "SIGTERM");
        } else {
          process14.kill(process14.pid, signal);
        }
      }
    };
    this.hook = () => {
      process14.once("exit", () => this.exit());
      for (const signal of signals_default) {
        try {
          process14.once(signal, () => this.exit(signal));
        } catch {
        }
      }
    };
...
async function getBlip(serial) {
  return request(serial, { method: "GET" });
}
async function createBlip(content) {
  return request("", {
    method: "POST",
    body: JSON.stringify({ content })
  });
}
async function updateBlip(serial, content) {
  return request(serial, {
    method: "PUT",
    body: JSON.stringify({ content })
  });
}
async function deleteBlip(serial) {
  return request(serial, {
    method: "DELETE"
  });
}
...
var NONE = "none";
var terminalColumns = () => {
  const { env: env3, stdout, stderr } = process16;
  if (stdout?.columns) {
...
var sanitizeOptions = (options) => {
  if (options.fullscreen && process16?.stdout) {
    let newDimensions = [process16.stdout.columns, process16.stdout.rows];
...
function showBanner() {
  ...
  console.log(source_default.gray("  for sumitsute.com/blip"));
  console.log();
}
function formatTimeAgo(dateString) {
  ...
  if (diffMins < 1) return source_default.green("now");
  if (diffMins < 60) return source_default.gray(`${diffMins}m`);
  if (diffHours < 24) return source_default.gray(`${diffHours}h`);
  if (diffDays < 7) return source_default.gray(`${diffDays}d`);
  return source_default.gray(date.toLocaleDateString());
}
...
function renderBlipBox(blip) {
  const content = boxen(
    `${source_default.cyan.bold(blip.blip_serial)} ${source_default.gray("\u2502")} ${source_default.white(blip.content)}

${source_default.gray("Created: ")}${formatFullDate(blip.created_at)}`,
    {
      padding: 1,
      margin: { top: 0, bottom: 0, left: 2, right: 2 },
      borderStyle: "round",
      borderColor: "cyan",
      title: source_default.bold("blip"),
...
function renderBlipsTable(blips) {
  if (blips.length === 0) {
    console.log();
    console.log(source_default.gray("  No blips yet. Create one with:"));
    console.log(source_default.gray("  ") + source_default.cyan('blip "your thought here"'));
    console.log();
    return;
  }
  const table = new import_cli_table3.default({
    head: [source_default.gray("serial"), source_default.gray("content"), source_default.gray("ago")],
    colWidths: [8, 50, 8],
    style: {
      head: [],
      border: ["gray"],
  ...
  for (const blip of blips) {
    table.push([
      source_default.cyan.bold(blip.blip_serial),
      blip.content.length > 47 ? blip.content.substring(0, 44) + "..." : blip.content,
      formatTimeAgo(blip.created_at)
    ]);
  }
  console.log();
  console.log(source_default.bold(`  Blips (${blips.length})`));
  console.log();
  console.log(table.toString());
  console.log();
}
function renderSuccessBox(message, subtitle) {
  const content = subtitle ? `${source_default.green("\u2714")} ${message}
${source_default.gray(subtitle)}` : `${source_default.green("\u2714")} ${message}`;
  console.log();
...
function renderErrorBox(message, hint) {
  const content = hint ? `${source_default.red("\u2716")} ${message}
${source_default.gray(hint)}` : `${source_default.red("\u2716")} ${message}`;
  console.log();
  console.log(boxen(content, {
    padding: { top: 0, bottom: 0, left: 1, right: 1 },
    margin: { top: 0, bottom: 0, left: 2, right: 2 },
...
function renderConfigBox(config2) {
  const content = `${source_default.bold("API URL:")} ${config2.url}
${source_default.bold("API Key:")} ${config2.key ? source_default.green("****configured") : source_default.red("not set")}`;
  console.log();
  console.log(boxen(content, {
    padding: 1,
    margin: { top: 0, bottom: 0, left: 2, right: 2 },
    borderStyle: "round",
    borderColor: "yellow",
    title: source_default.bold("configuration"),
    titleAlignment: "center"
  }));
  console.log();
}
...
// src/commands/new.ts
async function newCommand(content) {
  if (!hasApiKey()) {
  ...
  if (result.error) {
    spinner.fail(source_default.red(result.error));
    process.exit(1);
  }
  spinner.succeed(source_default.green(`Created ${result.data.blip.blip_serial}`));
  const blip = result.data.blip;
  console.log();
  console.log(source_default.gray("   ") + source_default.cyan.bold(blip.blip_serial) + source_default.gray(" \u2502 ") + source_default.white(blip.content) + source_default.gray(" \u2502 ") + formatTimeAgo(blip.created_at));
}
...
// src/commands/get.ts
async function getCommand(serial, options) {
...
async function editCommand(serial, content) {
  if (!hasApiKey()) {
    renderErrorBox("API key not configured", "Run: blip config set key <your-key>");
    process.exit(1);
  }
  ...
  spinner.succeed(source_default.green(`Updated ${serial}`));
  const blip = result.data.blip;
  console.log();
  console.log(source_default.gray("   ") + source_default.cyan.bold(blip.blip_serial) + source_default.gray(" \u2502 ") + source_default.white(blip.content));
}

// src/commands/delete.ts
import inquirer2 from "inquirer";
async function deleteCommand(serial, options) {
  if (!hasApiKey()) {
    renderErrorBox("API key not configured", "Run: blip config set key <your-key>");
    process.exit(1);
  }
    ...
    const answers = await inquirer2.prompt([
      {
        type: "confirm",
        name: "confirm",
        message: `Delete blip ${source_default.cyan(serial)}?`,
        default: false
      }
    ]);
    if (!answers.confirm) {
  ...
  spinner.succeed(source_default.green(`Deleted ${serial}`));
}

// src/index.ts
var SERIAL_PATTERN = /^[a-zA-Z0-9]{1,6}$/;
var showHelp = process.argv.includes("--help") || process.argv.includes("-h");
var showVersion = process.argv.includes("--version") || process.argv.includes("-V");
...
program2.command("rm <serial>").description("Delete a blip").option("-f, --force", "Skip confirmation").action(async (serial, options) => {
  await deleteCommand(serial, options);
});
program2.command("edit <serial> [content...]").description("Edit a blip").alias("e").action(async (serial, contentParts = []) => {
  await editCommand(serial, contentParts.join(" "));
});
program2.command("interactive").description("Interactive mode").alias("i").action(async () => {
  ...
  switch (action) {
    case "create": {
      ...
      const { content } = await inquirer3.prompt([
        {
          type: "input",
          name: "content",
    ...
    case "list":
      ...
      break;
    case "get": {
      const { serial } = await inquirer3.prompt([
        {
          type: "input",
          name: "serial",
          message: "Blip serial:",
          validate: (input) => SERIAL_PATTERN.test(input) || "Invalid serial format"
        }
      ]);
    ...
    case "config": {
      ...
      renderConfigBox(config2);
      const { setting } = await inquirer3.prompt([
        {
          type: "list",
          name: "setting",
    ...
    case "exit":
      console.log(source_default.gray("Bye!"));
      break;
  }
});
program2.command("config").description("Manage configuration").argument("[action]", "set or list").argument("[key]", "config key (key or url)").argument("[value]", "config value").action(async (action, key, value) => {
...
async function handleDefaultCommand(args, jsonMode) {
  const arg = args.join(" ");
  if (args.length === 0) {
...
program2.argument("[args...]", "Serial to view, or content to create").option("--json", "Output as JSON (for list/get)").action(async (args, options) => {
