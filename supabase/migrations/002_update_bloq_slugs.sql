...
-- Wrapped in table-existence checks so it's safe to re-run after tables were dropped.

DO $$
BEGIN
    -- Update bloq_views table (only if it still exists)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'bloq_views') THEN
    ...
    -- Update claps table (only if it still exists)
